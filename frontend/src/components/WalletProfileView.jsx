...
const WalletProfileView = ({ walletAddress, profileHint = {}, onBack, onCoinClick, onOpenPosition }) => {
  ...
  const handleBellClick = async () => {
    ...
    if (nextState) {
      const perm = await maybeEnableNotifications();
      if (perm === 'granted') {
        ...
        showToast('Turn notifications on in Settings to get trade alerts');
  ...
  // Shared gesture (hooks/useSwipeBack.js). This page allows the drag to start
  // anywhere, not just the screen edge.
  const swipeBack = useSwipeBack({
    onBack,
    enabled: !showAnalyticsModal && !showRenameModal,
    edgeWidth: 0,
    classPrefix: 'wpv',
  });
  const closeWithSlide = swipeBack.closeWithSlide;
